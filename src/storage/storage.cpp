...
int Storage::Run(int max_wait)
{
    ...
    { // Lock for write access shared region mutex
        ...
        {
            next_timestamp = shared_register[region_id].timestamp + 1;
            in_use_key = shared_register[region_id].shm_key;
            shared_register[region_id].shm_key = shm_key;
            shared_register[region_id].timestamp = next_timestamp;
        }
    }
